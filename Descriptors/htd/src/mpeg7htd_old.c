...
Mpeg7HTDDescriptor *CreateMpeg7HTDDescriptor(int hei, int wid, int side, int scale, int orientation, 
	...
	float *f_r = (float *) malloc(sizeof(float) * hei * wid * scale * orientation);
	float *f_i = (float *) malloc(sizeof(float) * hei * wid * scale * orientation);
	...
	for(s=0,i=0,j=0;s<scale;s++) {
		for(o=0;o<orientation;o++) {
			...
			for(h=0;h<hei;h++) {
				for(w=0;w<wid;w++) {
					float v = ComplexMagnitude(f_r[j],f_i[j]);
					sum += v;
					j++;
				}
			}
...
Image *GetIntensityRGBImage(CImage *cimg) {
...
Mpeg7HTDDescriptor *ExtractMpeg7HTDDescriptor(CImage *rgbImage) {
	...
	Matrix *img, *F_r, *F_i;	
	Image *gray = GetIntensityRGBImage(rgbImage);
	double Ul, Uh;

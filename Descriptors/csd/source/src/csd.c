...
VisualFeature *ComputeHistograms(Property *p, int nrows, int ncols, int npixels, int *npoints)
{
    ...
    *npoints = 0;
    ...
    for (i = 0; i <= (nrows - e); i = i + k) {
        ...
        //printf("linha atual=%ld\n", i);

        for (j = 0; j <= (ncols - e); j = j + k) {

            //printf("ultima janela: [%d,%d]\n", j, i);
            ...
            for(c=0; c<SIZE; c++){
                ...
                //printf("%d ",vf->colorH[c]);
            }
            //printf("\n");
        }

    }
    //printf("terminou com janela em: (%d,%d)\n", j, i);
    //printf("ncols: %d\t nrows= %d\n", ncols, nrows);
    //printf("npixels=%d, p2=%d\t k=%d\t e=%d\n", npixels, p2, k, e);

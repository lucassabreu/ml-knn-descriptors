...
CImage* CImageRGBtoLab(CImage* cimg)
{
    ...
    int n = cimg->C[0]->ncols*cimg->C[0]->nrows;

    int i;

    CImage* lab = CreateCImage(cimg->C[0]->ncols, cimg->C[0]->nrows);
...
CImage* CImageLabtoRGB(CImage* cimg)
{
    ...
    CImage* rgb = CreateCImage(cimg->C[0]->ncols, cimg->C[0]->nrows);
...
Image *CFeatureGradient(CImage *cimg, int maxval)
{
    ...
    Features* mslp = MSLowPassFeats(l, 3);

    for(i = 0; i < l->nfeats; i++)
        for(j = 0; j < l->nelems; j++)
            l->elem[j].feat[i] = l->elem[j].feat[i]/255;

    Features* f = ConcatFeatures(l,mslp);
    ...
    for (p=0; p < n; p++)
    {
        ...
        float x =(maxval*gx)/(float)f->nfeats;
        float y =(maxval*gy)/(float)f->nfeats;

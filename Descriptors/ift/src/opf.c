                ...
                if (pathval[p] < pathval[q])
                {
                    if (sg->node[p].truelabel!=sg->node[q].truelabel)
                        ...
                        if (!PrecomputedDistance)
                            weight = (int)ArcWeight(DistFun(sg->node[p].feat,sg->node[q].feat,sg->nfeats), sg->df);
                        else
...
void ClassifyCompGraph(Subgraph *sgtrain, Subgraph *sg)
{
    ...
    for (i = 0; i < sg->nnodes; i++)
    {
        ...
        {
            if (!PrecomputedDistance)
                weight = (int)ArcWeight(DistFun(sgtrain->node[j].feat,sg->node[i].feat,sg->nfeats), sgtrain->df);
            else
...
Image *ImageClassCompGraph(Subgraph *sg, Features *f)
{
    ...
    for (q=0; q < n; q++)
    {
        ...
        {
            weight = ArcWeight(DistFun(f->elem[q].feat,sg->node[0].feat,sg->nfeats), sg->df);
            ...
            for (p=1; p < sg->nnodes; p++)
            {
                ...
                {
                    weight = ArcWeight(DistFun(f->elem[q].feat,sg->node[p].feat,sg->nfeats), sg->df);
...
// Classify nodes of evaluation/test set By bayes
Scene **ClassifyBayesforMajorityVote( Subgraph *sg, Features3 *f, Scene *mask ) {
  CNode  *node = CreateCNode( sg );
...
Scene **SceneClusterforMajorityVote( Subgraph *sg, Features3 *f, Scene *mask, int tiebreak )
{
  ...
  for ( p = 0; p < n; p++ ) {
    if ( mask->data[ p ] ) {
      ...
      dx = ( VoxelX( result[ 0 ], p ) - VoxelX( result[ 0 ], pred ) ) * ( VoxelX( result[ 0 ], p ) - VoxelX( result[ 0 ], pred ) );
      dy = ( VoxelY( result[ 0 ], p ) - VoxelY( result[ 0 ], pred ) ) * ( VoxelY( result[ 0 ], p ) - VoxelY( result[ 0 ], pred ) );
      dz = ( VoxelZ( result[ 0 ], p ) - VoxelZ( result[ 0 ], pred ) ) * ( VoxelZ( result[ 0 ], p ) - VoxelZ( result[ 0 ], pred ) );
      dist = EuclDist( sg->node[ pred ].feat, node->feat, sg->nfeats );
      ( result[ 0 ] )->data[ p ] = sg->node[ pred ].label;
      ( result[ 1 ] )->data[ p ] = 1000000 / sqrt( ( float ) dx + dy + dz );
      ( result[ 2 ] )->data[ p ] = sg->node[ pred ].dens / ( 20 * dist );
      //printf( "scene:%d, feature:%d\n", ( result[ 1 ] )->data[ p ], ( result[ 2 ] )->data[ p ] );
      nvoxels++;
    }
  }
  DestroyCNode( &node );
...
void CreateSymArcs(Subgraph *sg)
{
    ...
    for (i=0; i < sg->nnodes; i++)
    {
        for (j=0; j < sg->nnodes; j++)
        {
            if (i!=j)
            {
                dist = DistFun(sg->node[i].feat,sg->node[j].feat,sg->nfeats);
                if (dist <= sg->df)
...
void NodeArcs(Subgraph *sg, CNode *node)
{
    ...
    {
        if (!PrecomputedDistance)
            node->dist[sg->bestk] = DistFun(node->feat,sg->node[j].feat,sg->nfeats);
        else
...
void NodeArcsByTrueLabel(Subgraph *sg, CNode *node, int label)
{
    ...
    for (j=0; j < sg->nnodes; j++)
    {
        ...
        if (!PrecomputedDistance) node->dist[sg->bestk] = DistFun(node->feat,sg->node[j].feat,sg->nfeats);
...
void TestNodePD(Subgraph *sg, CNode *node)
{
    ...
    {
        if (!PrecomputedDistance)
            dist = DistFun(node->feat,sg->node[node->adj[i]].feat,sg->nfeats);
        else
            ...
            for (j=0; j < (*sg)->nnodes; j++)
                if ((i!=j)&&((*sg)->node[j].status!=NIL))  // consistent node
                {
                    if ((*sg)->node[i].truelabel!=(*sg)->node[j].truelabel)
                        if (DistFun((*sg)->node[i].feat,(*sg)->node[j].feat,(*sg)->nfeats)==0.0) // i is not consistent with j, then mark j as inconsistent node
                        {
...
Subgraph* SplitSubgraphByTrueLabel(Subgraph* sg, int label)
{
    ...
    for(i = 0; i < sg->nnodes; i++)
    {
        ...
        {
            sgresult->node[i0].feat = AllocFloatArray(sg->nfeats);
...
int CountNodesByTrueLabel(Subgraph* sg, int label)
{
    if(sg == NULL)
    {
        Error("Subgraph NULL ", "CountNodesByTrueLabel");
    }

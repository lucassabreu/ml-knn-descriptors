...
//###-INICIO
unsigned mapHist[] = { 13, 0, 0, 0, 11, 12, 12, 12, 13, 0, 0, 0, 1, 2, 2, 2, 3,
                       0, 0, 0, 1, 2, 2, 2, 3, 3, 0, 0, 1, 2, 2, 2, 3, 3, 3, 4,
                       5, 6, 6, 6, 7, 7, 7, 8, 9, 10, 10, 10, 7, 7, 7, 8, 9,
                       10, 10, 10, 7, 7, 7, 8, 9, 10, 10, 30, 13, 13, 11, 11,
                       11, 12, 12, 12, 13, 13, 13, 11, 11, 12, 12, 12, 13, 13,
                       13, 11, 11, 12, 12, 12, 17, 13, 13, 14, 15, 16, 16, 22,
                       17, 17, 18, 19, 20, 21, 22, 22, 23, 23, 18, 24, 25, 26,
                       27, 27, 23, 23, 23, 24, 28, 29, 30, 30, 23, 23, 23, 28,
                       28, 29, 30, 30, 34, 34, 31, 31, 31, 32, 33, 33, 34, 34,
                       34, 31, 31, 32, 33, 33, 34, 34, 34, 31, 31, 32, 33, 33,
                       39, 34, 34, 35, 36, 37, 38, 38, 39, 39, 40, 41, 42, 43,
                       44, 44, 39, 39, 40, 45, 46, 47, 48, 48, 39, 39, 45, 45,
                       49, 50, 51, 51, 23, 23, 45, 49, 49, 50, 51, 51, 53, 53,
                       53, 52, 52, 52, 52, 52, 53, 53, 53, 54, 52, 52, 52, 58,
                       53, 53, 53, 54, 52, 52, 52, 58, 53, 53, 53, 54, 55, 56,
                       57, 58, 59, 59, 59, 60, 61, 62, 63, 58, 59, 59, 59, 60,
                       61, 62, 63, 63, 59, 59, 59, 60, 49, 50, 51, 51, 59, 59,
                       45, 49, 49, 50, 51, 51};
//###-FIM
...
unsigned h1, h2, h3;
  ...
  for(i=0; i<n; i++){
//###-INICIO
    cL = (float)img->C[0]->val[i];
    ca = (float)img->C[1]->val[i];
    cb = (float)img->C[2]->val[i];
    RGB2Lab(&cL,&ca,&cb);

    if(cb < -99.9) cb = -99.9;

    h1 = cL / 25;
    if(h1 == 4) h1 = 3; // if cL == 100
    h2 = (ca + 100.0) / 25;
    h3 = (cb + 100.0) / 25;

    color[i] = mapHist[64*h1 + 8*h2 + h3]; // com Luminancia
//    color[i] = (8*h2 + h3) & 63; // sem Luminancia

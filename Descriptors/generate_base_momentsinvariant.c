...
int main(int argc, char** argv)
{
   ...
   FILE *fout = fopen("MomentsInvariant.out","wt");
   for(count=0;count<n;count++) {
      ...
      fv[count] = MomentInvariant_ExtractionAlgorithm(img1);
  ...
  fout = fopen("MomentsInvariant.bin","wb");
  fwrite(&n, sizeof(unsigned), 1, fout);
  for(i=0;i<n;i++) {
      fprintf(stderr,"computing distances (%d/%d))...    \r", i+1, n);
      DistanceValue[i][i] = 0.0;
      for(j=i+1;j<n;j++)
        DistanceValue[i][j] = DistanceValue[j][i] = (float)MomentInvariant_DistanceAlgorithm(fv[i],fv[j]);

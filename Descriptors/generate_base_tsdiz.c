...
int main(int argc, char** argv)
{
   ...
   for(count=0;count<n;count++) {
      ...
      fv[count] = TSDIZ_ExtractionAlgorithm(img1,50);
  ...
  fout = fopen("TSDIZ.bin","wb");
  fwrite(&n, sizeof(unsigned), 1, fout);
  for(i=0;i<n;i++) {
      fprintf(stderr,"computing distances (%d/%d))...    \r", i+1, n);
      DistanceValue[i][i] = 0.0;
      for(j=i+1;j<n;j++)
        DistanceValue[i][j] = DistanceValue[j][i] = (float)TSDIZ_SimilarityAlgorithm(fv[i],fv[j]);
